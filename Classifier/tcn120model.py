...
class TCN120Model1(nn.Module):
        def __init__(self, device):
            super(TCN120Model1, self).__init__()
            self.name = 'tcn120model1'
            self.device = device
            self.input_size = 20 * 108 * 192
            self.batch_dim = 10
            self.fcdrop = nn.Dropout(0.25)
            ...
            self.fc1 = nn.Linear(6912, 1728)
            self.fc2 = nn.Linear(1728, 512)
            ...
            self.unit4 = UnitTCN(16, 24, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(24, 20, 54, 48))
            self.unit4_1 = UnitTCN(24, 24, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(24, 20, 27, 48))

            self.unit5 = UnitTCN(24, 32, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(32, 20, 27, 48))
            self.unit5_1 = UnitTCN(32, 32, kernel_size=(3, 3, 5), stride=1, dropout=0.3, d_in=(32, 20, 27, 24))

            self.unit6 = UnitTCN(32, 40, kernel_size=(3, 3, 5), stride=1, dropout=0.3, d_in=(40, 20, 27, 24))
            self.unit6_1 = UnitTCN(40, 40, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(40, 10, 27, 24))

            self.unit7 = UnitTCN(40, 64, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(64, 10, 27, 24))
            self.unit7_1 = UnitTCN(64, 64, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(64, 10, 13, 24))

            self.unit8 = UnitTCN(64, 88, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 10, 13, 24))
            self.unit8_1 = UnitTCN(88, 88, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 10, 13, 12))

            self.unit9 = UnitTCN(88, 102, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 10, 13, 12))
            self.unit9_1 = UnitTCN(102, 102, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 10, 6, 12))

            self.unit10 = UnitTCN(102, 128, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 10, 6, 12))
            self.unit10_1 = UnitTCN(128, 128, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 10, 6, 6))

            self.unit11 = UnitTCN(128, 144, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 10, 6, 6))
            self.unit11_1 = UnitTCN(144, 144, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 5, 6, 6))

            self.unit12 = UnitTCN(144, 160, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 5, 6, 6))
            self.unit12_1 = UnitTCN(160, 160, kernel_size=(3, 3, 3), stride=1, dropout=0.3, d_in=(96, 2, 6, 6))

            self.fc1 = nn.Linear(11520, 2880)
            self.fc2 = nn.Linear(2880, 512)
            self.fc3 = nn.Linear(512, 120)
        ...
        def predict(self, x):
            ...
            x = F.avg_pool3d(self.unit4(x), kernel_size=(1, 2, 1)) # 20x27x48
            x = self.unit4_1(x)

            x = F.avg_pool3d(self.unit5(x), kernel_size=(1, 1, 2)) # 20x27x48
            x = self.unit5_1(x)

            x = F.avg_pool3d(self.unit6(x), kernel_size=(2, 1, 1)) # 10x27x24
            x = self.unit6_1(x)

            x = F.avg_pool3d(self.unit7(x), kernel_size=(1, 2, 1)) # 10x13x24
            x = self.unit7_1(x)

            x = F.avg_pool3d(self.unit8(x), kernel_size=(1, 1, 2)) # 10x13x12
            x = self.unit8_1(x)

            x = F.avg_pool3d(self.unit9(x), kernel_size=(1, 2, 1)) #10x6x12
            x = self.unit9_1(x)

            x = F.avg_pool3d(self.unit10(x), kernel_size=(1, 1, 2)) #10x6x6
            x = self.unit10_1(x)

            x = F.avg_pool3d(self.unit11(x), kernel_size=(2, 1, 1)) #5x6x6
            x = self.unit11_1(x)

            x = F.avg_pool3d(self.unit12(x), kernel_size=(2, 1, 1)) #2x6x6
            x = self.unit12_1(x)
